...
describe("getFen test", () => {
    ...
    test("Fen integration test", () => {
        ...
        expect(getFen(variation, 3)).toBe("rnbqkbnr/pppp1ppp/8/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R b KQkq - 1 2")
    });
});
...
describe("PGN Parser Test", () => {
    describe("Test PGN Annotations", () => {
        ...
        test.each(annotationTable)("Annotations Single Variation", (annotations, expected) => {
        ...
        test.each(annotationTable)("Annotations Nested", (annotations, expected) => {
        ...
        test("Annotations Empty", () => {
            const testPgn = "1. e4 e5 {Fake Comment! Position is =} (1... Nf6 {Alekhine!?!?!})";
            const variation = loadPgn(testPgn, startFen);
    ...
    describe("Test PGN Arrows", () => {
        const arrowTable: Array<[string, Array<[Square, Square, string]>]> = [
            ["[f8c5red f8e7blue     f8b4green]", [["f8", "c5", "red"], ["f8", "e7", "blue"], ["f8", "b4", "green"]]],
        ...
        test("Arrows empty", () => {
            const testPgn = "1. e4 e5 {[Fake Arrows] [f8 e6]} (1... Nf6 {Alek([hine])!?!?!})";
            const variation = loadPgn(testPgn, startFen);
    ...
    describe("Test PGN Variations", () => {
        test("Test Single Variation", () => {
            ...
            expect(variation).not.toBeNull();
            expect(variation?.start).toEqual("rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2");
        ...
        test("Test Nested Variation", () => {
            ...
            expect(variation?.moves[0]).toMatchObject({
                ...
                moveNumber: 2,
                color: "w",
                fenAfter: "rnbqkbnr/pppp1ppp/8/4p3/4P3/2N5/PPPP1PPP/R1BQKBNR b KQkq - 1 2"
            });
            expect(variation?.parentVariation?.id).toStrictEqual(mainVar.moves[2].variation?.id);
        });
    });

    test("Test PGN Integration", () => {
        const testPgn = ("2...d6 {Philidor} (2...Nc6 [g8f6blue] {()[Tricky] Comment()} (2...Nf6) 3. Bb5 {Spanish (Ruy) Opening} Nf6 {Berlin} 4. d3!? " +
            "[f8c5red f8e7blue] Bc5! (4...Be7?! (4...g6)) 5. c3 {Tabia} 0-0 6. 0-0!$15 [h2h4red g2g4orange] {Comment!}) 3. d4 (3. Bc4) Bg4?"
        );
        const mainVar = loadPgn(testPgn, "rnbqkbnr/pppp1ppp/8/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R b KQkq - 1 2");
        expect(mainVar.moves[0].variation?.moves[7]).toMatchObject({
            move: "0-0",
            annotation: "! &eplus;",
            comment: "Comment!",
            arrows: [["h2", "h4", "red"], ["g2", "g4", "orange"]],
            moveNumber: 6,
            color: "w",
            fenAfter: "r1bq1rk1/pppp1ppp/2n2n2/1Bb1p3/4P3/2PP1N2/PP3PPP/RNBQ1RK1 b - - 2 6",
        })
    });
});